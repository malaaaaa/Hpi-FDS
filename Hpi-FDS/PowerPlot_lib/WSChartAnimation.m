...
- (void)dataAnimateWithDuration:(NSTimeInterval)duration
    ...
    if (update != nil) {
        void (^updateHandlerCopy)(NAFloat, id) = Block_copy(update);
    ...
    for (NSUInteger i=0; i<[self count]; i++) {
        ...
        [oldData addObject:[[[currentCtrl dataD] copy] autorelease]];
    ...
    for (NSUInteger i=0; i<[self count]; i++) {
        ...
        WSCoordinate *coordX = [currentCtrl coordX];
        WSCoordinate *coordY = [currentCtrl coordY];
        [newData addObject:[[[currentCtrl dataD] copy] autorelease]];
