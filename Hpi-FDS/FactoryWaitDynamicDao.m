...
+(NSMutableArray *)getMidDate:(NSDate *)stringTime:(NSString *)factoryName
{
   ...
   TbFactoryState *tbS=[TbFactoryStateDao getStateBySql:factoryName :stringTime];
    ...
    NSMutableArray *arr1= [ [NSMutableArray alloc] initWithObjects:
                       ...
                       unies,
                       s,
                       tf1.MAXSTORAGE==0?@"":  [NSString stringWithFormat:@"%.2f",tf1.MAXSTORAGE/10000.00   ]  ,
                       tf1.CHANNELDEPTH   , nil];
    [d addObject:arr1];
    NSMutableArray *arr2= [[NSMutableArray alloc] initWithObjects:
       tbS.ELECGENER==0?@"":[NSString stringWithFormat:@"%.2f", tbS.ELECGENER/10000.00 ] ,
        monthP==0?@"": [NSString stringWithFormat:@"%.2f",monthP/10000.00 ],
       yeasP==0?@"": [NSString stringWithFormat:@"%.2f",yeasP/10000.00],
        monthE==0?@"": [NSString stringWithFormat:@"%.2f",monthE/10000.00]  ,
        yeasE==0?@"": [NSString stringWithFormat:@"%.2f", yeasE/10000.00],
                           nil ];

    [d addObject:arr2];
    NSMutableArray *arr3= [[NSMutableArray alloc] initWithObjects:
                         tbS.STORAGE7==0?@"":   [NSString stringWithFormat:@"%.2f",tbS.STORAGE7/10000.00 ],
