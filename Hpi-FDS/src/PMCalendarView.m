...
-(id)initInstence:(CGRect)frame:(NSMutableArray *)zxDate
{
    ...
    zxdate=zxDate;
    NSLog(@"PMCalendarView  zxdate======== [%d]",[zxdate count]);
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    self.daysView =   [[PMDaysView alloc] initInstense:self.bounds :zxdate];
...
- (void) tapHandling: (UIGestureRecognizer *)recognizer
{
    ...
    if(CGRectContainsPoint(leftArrowRect, point)) 
    {
        ...
        NSLog(@"User tapped the prevMonth button=======================================");
    ...
    {
          NSLog(@"User tapped the nextMonth button=======================================");
...
-(id)initInstense:(CGRect)frame:(NSMutableArray *)ZXDate
{
    ...
    NSLog(@"PMDaysView  zxDate======[%d]",[zxDate count]);
    return [self initWithFrame:frame];
}
			...
			if(dayNumber >= (weekdayOfFirst-1) && day <= numDaysInMonth) {
                ...
                else 
                {
                   ...
                   color = [UIColor whiteColor];
                    if ([zxDate count] >0) {
                        //合计
                        if ([[zxDate objectAtIndex:0] isEqualToString:@"H"]) {
                            for (int a=1; a<[zxDate count]; a++) {
                                if (day==[[zxDate objectAtIndex:a] integerValue]) {
                                    NSString *mark=[[zxDate objectAtIndex:a] substringFromIndex:[[zxDate objectAtIndex:a] length]-1];
                                    if (  [mark  isEqualToString: @"P" ])
                                        color =[UIColor blueColor];
                                    if ([mark  isEqualToString:  @"S"])
                                        color =[UIColor redColor];
                                }
                            }
                        }
                        //共计
                         if ([[zxDate objectAtIndex:0] isEqualToString:@"T"]) {   
                             for (int a=1; a<[zxDate count]; a++) {
                                  if (day==[[zxDate objectAtIndex:a] integerValue]) {
