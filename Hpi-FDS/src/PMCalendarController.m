...
- (void) initializeWithSize:(CGSize) size
{
    ...
    self.digitsView =  [[PMCalendarView alloc] initInstence:CGRectInset(calendarRect
                                                                        , innerPadding.width
                                                                        , innerPadding.height) :ZXDate];
    ...
    self.digitsView.period = [PMPeriod oneDayPeriodWithDate:[[NSDate alloc] initWithTimeInterval:8*60*60 sinceDate:[f dateFromString:startDate]]];//@"2012-08-17"
 ...
 // self.digitsView.period = [PMPeriod oneDayPeriodWithDate:[NSDate date]];
  //  NSLog(@"self.digitsView.period[%@]",self.digitsView.period);
