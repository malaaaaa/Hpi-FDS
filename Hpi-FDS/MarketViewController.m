...
-(void)loadHpiGraphView{
    ...
    date=minDate;
    NSMutableArray *resultArray=[TmIndexinfoDao getTmIndexinfoByName:stringType startDay:date Days:graphData.xNum];
    for (int i=0; i<[resultArray count]; i++) {
        TmIndexinfoMore *tminfo= [resultArray objectAtIndex:i];
    ...
    //单张表格两条线，写入pointArray2
    if([stringType isEqualToString: @"BJ_PRICE"]){
        NSMutableArray *resultArray=[TmIndexinfoDao getTmIndexinfoByName:@"BJ_INDEX" startDay:date Days:graphData.xNum];
        for (int i=0; i<[resultArray count]; i++) {
            TmIndexinfoMore *tminfo= [resultArray objectAtIndex:i];
            HpiPoint *point=[[HpiPoint alloc]init];
            point.x=tminfo.days;
    ...
    if([stringType isEqualToString: @"QHD_GZ"]){
        NSMutableArray *resultArray=[TmIndexinfoDao getTmIndexinfoByName:@"QHD_SH" startDay:date Days:graphData.xNum];
        for (int i=0; i<[resultArray count]; i++) {
            TmIndexinfoMore *tminfo= [resultArray objectAtIndex:i];
            HpiPoint *point=[[HpiPoint alloc]init];
            point.x=tminfo.days;
